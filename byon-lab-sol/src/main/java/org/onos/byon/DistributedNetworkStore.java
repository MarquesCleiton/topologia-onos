...
public class DistributedNetworkStore
    ...
    public boolean addHost(String network, HostId hostId) {
         ...
         */
        Versioned<Set<HostId>> existingHosts = checkNotNull(networks.get(network),"Network %s does not exist", network);
        Set<HostId> hosts = existingHosts.value();
    ...
    public Set<HostId> getHosts(String network) {
        ...
        return checkNotNull(networks.get(network).value(),
